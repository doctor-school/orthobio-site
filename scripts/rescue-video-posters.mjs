 ...
 * anywhere). With `--upload` it PUTs the derivatives to S3 via the AWS CLI —
 * new objects under a prefix of its own, never a delete and never a sync, since
 * the bucket is live paid infra holding 2.3k rescued archive objects. Reads its
 * credentials from the env var NAMES in infra/terraform/README.md
 * (`TIMEWEB_S3_ENDPOINT`, `TIMEWEB_S3_BUCKET`, `TIMEWEB_S3_ACCESS_KEY`,
 * `TIMEWEB_S3_SECRET_KEY`); values come from `terraform output` and live
 ...
 * dimensions, which is what the `posters:` section of docs/assets-manifest.yaml
 * and the `poster:` blocks in src/content/congress/*.yaml are written from.
 */
...
const outDir = (() => {
  ...
  return path.resolve(i === -1 ? '.rescue/posters' : process.argv[i + 1]);
})();
...
for (const video of videos) {
  ...
  // One object per video even if two years cite the same upload.
  if (seen.has(ident.key)) {
    index.find((e) => e.key === ident.key).years.push(video.year);
    continue;
  }
  seen.add(ident.key);
   ...
   * the 480×360 letterboxed original, bars and all, at an aspect ratio the card
   * does not have. Cropping first makes 16:9 unconditional and leaves the
   * downscale free to stop at the source's own width.
   */
...
if (doUpload) {
  ...
  for (const entry of index) {
    ...
    if (!res.ok) throw new Error(`PUT ${entry.object.s3_key}: HTTP ${res.status} ${await res.text()}`);
  }

  // Read back what the bucket actually SERVES. A PUT that "succeeded" against a
  // wrong key, or into a bucket whose public-read flag is off, leaves 23 cards
  // silently broken behind a green run.
  let verified = 0;
  for (const entry of index) {
    ...
    else console.error(`  ! ${entry.object.s3_key}: HTTP ${res.status}, ${body?.length ?? 0} B (expected ${entry.object.bytes})`);
  }
  console.log(`uploaded and verified ${verified}/${index.length} objects at ${s3.endpoint}/${s3.bucket}/posters/`);
  if (verified !== index.length) process.exitCode = 1;
}
