...
> What is decided and live, which owner actions completed the launch, and the
> exact GitHub configuration the deploy consumes. Modeled on
...
both `orthobio.ru` and `www.orthobio.ru`. It was issued on 2026-07-31 **before**
the A/CNAME cutover through a manual DNS-01 challenge: the owner added the two
temporary TXT values generated by certbot in Beget. The ECDSA certificate
expires **2026-10-29**. `provision.sh --enable-production` refused to enable the
vhost until OpenSSL confirmed both names in that certificate.

After public DNS reached the Timeweb origin, the lineage was reconfigured to the
nginx authenticator with `http-01`. `certbot renew --dry-run` succeeded on
2026-07-31, `certbot.timer` is enabled and active, and the obsolete manual hooks
were removed from the renewal configuration. Future renewals require no owner
DNS action.
...
This is **not** guarded by prose alone. In preview mode,
`SITE_HOST=new.orthobio.ru` requires `SITE_INDEXABLE=false`, and the deploy's
live check reads the response header from the resolve-pinned preview vhost.
Production has been active since 2026-07-31:
`SITE_HOST=orthobio.ru` and `SITE_INDEXABLE=true`. The workflow accepts only
these two hostname/indexability pairs and has no fallback values. In production
mode the same resolve-pinned check requires the response to be indexable and
requires the live host, homepage canonical, robots sitemap URL, and sitemap root
URL to agree. The production vhost redirects `www` to the apex in one hop.

### Switchover checklist (Issue #6)

Completed on 2026-07-31; the owner applied DNS at 06:03 UTC, the production
smoke passed, and rollback was not required. The sequence remains the reusable
runbook:
...
7. Owner changes Beget records in one window: apex A
   ...
   `orthobio.ru` with TTL 600. Record the timestamp and keep the old A value as
   rollback evidence.
...
### A. Preview DNS A-record — **DONE 2026-07-27**
...
resolving globally. TLS followed immediately (see decision 2).
**`https://new.orthobio.ru/` remains the non-indexable preview URL**; the apex
has served the production site since the completed 2026-07-31 switchover.
...
### B. Domain switchover — **DONE 2026-07-31** (Issue #6)

Applied authoritative snapshot (2026-07-31, all Beget NS):

| Name | Before | Live after cutover |
| --- | --- | --- |
| `@` | A `45.67.58.226` | A `5.42.108.242`, TTL 300 |
| `www` | absent | CNAME `orthobio.ru`, TTL 600 |
| `_acme-challenge` | absent | absent; temporary issuance TXT removed |
| `_acme-challenge.www` | absent | absent; temporary issuance TXT removed |

The owner completed the Beget-only instruction: first the two temporary TXT
validations, then — after valid resolve-pinned TLS and a green production deploy
were confirmed — the A/CNAME changes above. No SSH or GitHub action was
delegated to the owner.

Rollback was not required. The retained procedure is the inverse DNS operation:
restore apex A `45.67.58.226` and remove the new `www` CNAME. New resolver
lookups should recover within the applicable record TTL (300 seconds for apex,
600 seconds for `www`), while already cached answers may take their remaining
TTL. Before any future rollback, recheck that the old operator origin is still
available.
...
| `/etc/nginx/sites-available/orthobio.ru` | `infra/nginx/orthobio.ru.conf` | `provision.sh`; enabled by `--enable-production` on 2026-07-31 after certificate/SAN checks. |
